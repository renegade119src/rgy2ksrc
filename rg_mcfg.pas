...
Procedure BoardEdit;
VAR MsgBase:pMsgBaseObj;

  function flagstate(var MsgBase:pMsgBaseObj):string;
  var s:string[5];
  begin
    s:='';
    with MsgBase^.Data^ do
    begin
      if HasFlag(MbRealName) then s := s+'R' else s := s+'-';
      if HasFlag(mbunhidden) then s:=s+'U' else s:=s+'-';
      if HasFlag(mbfilter) then s:=s+'A' else s:=s+'-';
      if HasFlag(mbprivate) then s:=s+'P' else s:=s+'-';
      if HasFlag(mbforceread) then s:=s+'F' else s:=s+'-';
    end;
(*
...
(*      reset(MBasesFile);*)
      mciallowed := FALSE;
      AllowContinue := TRUE;
      while (ii<=MsgBase^.BaseCount) and (not abort) and (not hangup) do
      begin
        MsgBase^.CurrentBase := ii-1;
        MsgBase^.LoadBase;
(*      seek(MBasesFile,ii-1); read(MBasesFile,memboard);*)

        s:='^0' + mn(ii,4)+' '+'^5'+mln(MsgBase^.Data^.Name,28)+' ^3;
...
(*      with memboard do begin*)
        ...
        begin
          case ltype of
            1:s:=s+copy('LEGQ',mbtype+1,1)+flagstate(MsgBase)+' ^9'+
...
(*      readboard:=-1; *){loadboard(1);  not needed }
    ...
    case c of
      ...
      'D':begin
            prt('Board number to delete? (1-'+cstr(MsgBase^.BaseCount)+') : '); inu(ii);
            if ((not badini) and (ii>=1) and (ii<=MsgBase^.BaseCount)) then
            begin
(*              readboard:=-1; loadboard(ii);*)
              With MsgBase^ do
              begin
                CurrentBase := ii; Loadbase;
                s:=general.msgpath+Data^.filename;
                print(^M^J'Message base: ^5' + Data^.name);
                if pynq('Delete this? ') then begin
                  if (Data^.mbtype in [1, 2]) {and (existdir(memboard.msgpath))} and
                     pynq('Remove directory '+Data^.msgpath+ ' ? ') then
                      purgedir(Data^.msgpath, TRUE);
...
Configuration
  Procedure EditMessageBases;
  Function  EditTextFile(FN:Str255):BOOLEAN;
...
var f1:file;
  ...
  begin
    ...
    if (i >= 0) and (i <= filesize(MBasesFile))  then begin
      ...
      if (badini) then
        q := 1;
      if (General.CompressBases) and ((filesize(MBasesFile) + q) > 1024) then
        begin
          print('You cannot have more than 1,024 bases with base number compression on.');
          pausescr(FALSE);
          q := 0;
        end;
      if (q >= 1) then
  ...
  function netflags(mb:boardrec):string;
  var s:string[6];
  begin
    s:='';
    with mb do begin
  ...
  procedure incolor(msg:astr; var i:byte);
  begin
