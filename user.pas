...
begin
  for i:=1 to length(cms)-1 do
    begin
      c := upcase(cms[i]);
      case c of
         '+':thisuser.ar := thisuser.ar + [cms[i + 1]];
         '-':thisuser.ar := thisuser.ar - [cms[i + 1]];
         '!':if (cms[i + 1] in thisuser.ar) then
               thisuser.ar := thisuser.ar - [cms[i + 1]]
             else
               thisuser.ar := thisuser.ar + [cms[i + 1]];
      end;
...
begin
  for i:=1 to length(cms)-1 do
    begin
      c:=upcase(cms[i]);
      case c of
         '+':thisuser.flags:=thisuser.flags+[tacch(cms[i+1])];
         '-':thisuser.flags:=thisuser.flags-[tacch(cms[i+1])];
         '!':acch(cms[i+1],thisuser);
      end;
    end;
  newcomptables;
  update_screen;
